...
|-----------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|------------------------------------------------|
| **Arduino Uno/Nano**         | Микроконтроллер для управления системой. Используется для обработки сигналов с датчиков и управления насосом.                                                   | 1              | Главное устройство управления.                 |
| **Датчик уровня жидкости**   | Датчик для определения уровня жидкости в ёмкости. Может быть ультразвуковым или емкостным в зависимости от потребностей.                                          | 1 или 2        | Для точного контроля уровня жидкости.          |
| **Реле**                     | Используется для включения и выключения насоса. Управляется Arduino.                                                                                             | 1              | Для управления мощными устройствами.           |
| **Насос**                    | Погружной насос для подачи жидкости в ёмкость. Может быть различных типов в зависимости от задачи (например, мембранный или центробежный).                        | 1              | Для подачи жидкости в ёмкость.                 |
| **Питание (Блок питания)**   | Источник питания для Arduino и всех компонентов системы. Рекомендуется использовать блок питания с возможностью регулировки напряжения.                         | 1              | Для питания системы.                           |
| **Провода и соединения**     | Набор проводов для подключения всех компонентов: Arduino, датчиков, реле, насоса и других элементов.                                                              | 1 комплект     | Для соединения компонентов системы.            |
| **Кнопки/выключатель**       | Для включения/выключения устройства вручную, если необходимо.                                                                                                      | 1              | Для управления включением системы.             |
| **3D модель корпуса**        | Спроектирован корпус устройства для защиты внутренних компонентов и обеспечения удобства использования. Модель может быть распечатана на 3D-принтере.            | 1              | Защищает устройство от внешних повреждений.    |
| **Диоды/Светодиоды**         | Для визуального отображения состояния работы устройства (например, индикаторы работы насоса или уровней жидкости).                                              | 2-3            | Для отображения состояния работы системы.      |
| **Клеммники/Разъёмы**        | Для удобства подключения проводов и компонентов, а также для обеспечения надежного соединения.                                                                  | 1 комплект     | Для подключения компонентов к системе.        |
| **Микросхема/Транзистор**    | Для управления мощностью, если реле не может справиться с нужной нагрузкой.                                                                                      | 1              | Для усиления сигнала для управления насосом.   |
